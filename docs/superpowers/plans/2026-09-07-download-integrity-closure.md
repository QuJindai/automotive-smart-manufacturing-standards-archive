...
- [x] Run local suites: Python 47 passed, two browser-only cases skipped; Node 52 passed. The production shell/normalizer is exercised for OIDC refresh failure and malformed gateway JSON as well as success/failure receipts.
- [ ] Run the two real Chromium/helper cases in Linux CI, not a mocked browser.
- [ ] Deploy reviewed gateway sources with rollback captured and release the executor only after green CI.
- [ ] Prove fresh production static and unknown-length jobs with matching independent Drive SHA-256.
- [ ] Prove a fresh production file larger than 100 MiB.
- [ ] Complete a controlled 10-file production batch and record durable evidence.
- [ ] Record deployed revision, function versions and exact release gates in a final release note.
