...
def upload_staged_resumable(staged: AssetResult, asset: dict, session_url: str, chunk_size: int = DEFAULT_CHUNK) -> AssetResult:
    ...
    try:
        total, digest = validate_file(
            path,
            asset.get("kind"),
            staged.bytes if staged.bytes > 0 else None,
            staged.sha256,
        )
        if total <= 0:
        ...
        if offset == total:
            return AssetResult(
                ...
                total,
                digest,
                "drive-resumable-local-resume",
                drive_ref={"name": filename, "size": total, "sha256": digest},
        ...
        if session:
            ...
            if result.status != "PASS" and result.error == "source size unavailable":
                ...
                if staged.status == "PASS" and staged.bytes > 0 and staged.sha256 and staged.artifact_path:
                    ...
                    result = AssetResult(
                        asset_id=str(raw.get("asset_id") or staged.asset_id or ""),
                        filename=str(raw.get("filename") or staged.filename or "download.bin"),
                        status="FAIL",
                        bytes=0,
                        sha256=None,
                        method="drive-resumable-local",
                        error="staged artifact metadata incomplete",
                        artifact_path=staged.artifact_path,
                        source_url_redacted=redact_url(str(raw.get("source_url") or "")) if raw.get("source_url") else None,
                    )
                else:
                    result = staged
        else:
