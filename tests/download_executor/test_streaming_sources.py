        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_HEAD(self):
                if self.path == "/redirect":
                    self._finish_empty(302, Location="/static")
                    return
                if self.path == "/static":
                    self.send_response(200)
                    if not fixture.unknown_length:
            ...
            def do_GET(self):
                ...
                if self.path == "/static":
                    ...
                    if requested_range and fixture.range_supported:
                        match = re.fullmatch(r"bytes=(\d+)-(\d*)", requested_range)
                        start = int(match[1])
                        end = int(match[2]) if match[2] else len(body) - 1
                        body = body[start:end + 1]
                        self.send_response(206)
                        self.send_header("Content-Range", f"bytes {start}-{end}/{len(fixture.body)}")
                    else:
                        self.send_response(200)
                    if fixture.unknown_length:
                    ...
                    try:
                        ...
                        pass  # A size probe can close without reading the full response.
                    return
                if self.path == "/dynamic":
            ...
            def do_PUT(self):
                ...
                fixture.put_starts.append(start)
